...
import Pagination, { paginate } from "@/app/api/search/Pagination"
      ...
      <section className="mt-6 flex">
        <section className="w-full">
          <div className=" flex items-center justify-between pb-5">
            <h2>
              <span className="font-semibold text-white leading-6 text-natural-6">
                Showing:
              </span>
            ...
            <Suspense fallback={<Loader />}>
              {jobs.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center">
                  <Loader />
                  <h3 className="headline-2 text-slate-300">Start Searching for Jobs!</h3>
                </div>
              )}
            </Suspense>
          </div>
          <section className="m-10 border border-transparent border-t-natural-2 bg-gray-900 dark:border-t-darkBG-3">
            <Suspense fallback={<Loader />}>
              <Pagination
                jobs={jobs.length}
